...
class SDDPStudyCase:
    ...
    def run_study(self):
        ...
        try:
            ...
            self.sddp_command = self.sddp_command._replace(
                id=case_id
            )
            ...
            while status not in psr.cloud.status.FINISHED_STATUS:  # type: ignore
                ...
                if status != previous_status:
                    print(
                        f"Case {self.sddp_command.id} status changed from {previous_status} to {status}."
                    )
                    previous_status = status
        ...
        if self.sddp_command.id:
            ...
            if str(status) == "ExecutionStatus.SUCCESS":
                ...
                self.client.download_results(
                    self.sddp_command.id, self.sddp_command.pathname, output_files, []
                )
