...
1. ~~**`psr_cloud_evoc_ibv.xml`**~~ — Added to `.gitignore`; `*.xml` pattern already prevents tracking.
2. ~~**Error handling in `PSRIOCommand.process_bin_to_dataframe`**~~ — Now raises `RuntimeError` instead of calling `sys.exit()`.
3. **`test.ipynb` / `test0.ipynb`** — Exploratory notebooks present but not integrated into any test suite.
4. ~~**No automated tests**~~ — Basic `pytest` test suite added under `tests/`.
5. ~~**`levels` field parsing**~~ — Validation added; invalid characters now raise `ValueError`.
6. ~~**`Download` command runs on main thread**~~ — Now threaded like `Run`/`RunDownload`.
7. ~~**`get_bus` returns `None`**~~ — `get_bus_agents` now skips plants without a bus mapping.
8. ~~**MRU history file**~~ — `load_history()` now prunes entries pointing to non-existent directories.
